...
#%%
def getVillagerInfo(page_source):
...
def acnhpop_df(villager_field_dict):
...
print(f'STEP 1 STARTING AT {NOW}')
page_source = acnGetPopData()
print(f'STEP 2 STARTING AT {NOW}')
villager_field_dict = getVillagerInfo(page_source)
print(f'STEP 3 STARTING AT {NOW}')
df = acnhpop_df(villager_field_dict)
print(f'STEP 4 STARTING AT {NOW}')
df_kag = kaggle_data()
print(f'STEP 5 STARTING AT {NOW}')
df_final = joinTables(df, df_kag)
print(f'STEP 6 STARTING AT {NOW}')
mysql(df_final)
print(f'DONE!!!! YAY!!! AS OF {NOW}')
